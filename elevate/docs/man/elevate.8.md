---
title: ELEVATE(8) elevate-zainium 0.2.13 | elevate-zainium
---

# NAME

`elevate`, `elevatedit` - execute a command as another user

# SYNOPSIS

`elevate` `-h` | `-K` | `-k` | `-V`\
`elevate` \[`-u` *user*\] \[`-g` *group*\] \[`-D` *directory*\] \[`-BbknS`\] \[`-i` | `-s`\] \[`VAR=value`\] \[<*command*>\]\
`elevate` `-v` \[`-BknS`\] \[`-u` *user*\]  \[`-g` *group*\]\
`elevate` `-l` \[`-BknS`\] \[`-U` *user*\] \[`-u` *user*\]  \[`-g` *group*\] \[command \[arg ...\]\]\
`elevate` `-e` \[`-BknS`\] \[`-u` *user*\] \[`-g` *group*\] file ...\
`elevatedit` \[`-BknS`\] \[`-u` *user*\] \[`-g` *group*\] file ...


# DESCRIPTION

`elevate` allows a user that is permitted to do so to execute a *command* as
another user (for example *root*). Permissions are specified by a security
policy specified in `/etc/elevators/elevate.toml` (see elevate.toml(5)).

Elevate is a safety oriented and memory safe re-implementation of the original
sudo implementation by Todd Miller, customized for Zainium OS.
...
Some care is taken to pass signals received by elevate to the child process,
even if that process runs in its own pseudo terminal.

On systems where elevate is the primary method of gaining superuser privileges, it is
imperative to avoid syntax errors in the `/etc/elevators/elevate.toml` file. Changes to this file
should be made using the viselev(8) utility which will ensure that no syntax errors
are introduced.

# OPTIONS

`-A`, `--askpass`
:   Normally, if elevate requires a password, it will read it from the user's terminal.  If the -A (askpass) option is specified, a (possibly graphical)
    helper program is executed to read the user's password and output the password to the standard output.  If the ELEVATE_ASKPASS environment variable is
    set, it specifies the path to the helper program. If no askpass program is available, elevate will exit with an error.
...
:   Run the given *command* in the background. It is not possible to use shell
    job control to manipulate background processes started by elevate. Most
...
:   Removes every cached session record for the user, regardless of where the
    command is executed. The next time elevate is run, authentication will take
...
:   When used without a command, invalidates the user's session record for
    the current session. The next time elevate is run, authentication will take
    place if the policy requires it.

    When used in conjunction with a *command* or an option that may require a
    password, this option will cause elevate to ignore the user's session
...
:   Avoid prompting the user for input of any kind. If any input is required for
    the *command* to run, elevate will display an error message and exit.
...
:   Use a custom authentication prompt with optional escape sequences. The
    ...
    The custom prompt will override the default prompt or the one specified by the ELEVATE_PROMPT environment variable.
...
:   Edit one or more files instead of running a command.  In lieu of a path name, the string "elevatedit" is used when consulting the security policy.  If the user is authorized by the policy, the following steps are taken:

    1. Temporary copies are made of the files to be edited with the owner set to the invoking user.

    2. The editor specified by the policy is run to edit the temporary files.  The elevate.toml policy uses the ELEVATE_EDITOR, VISUAL and EDITOR environment variables (in that order).  If none of ELEVATE_EDITOR, VISUAL or EDITOR are set, the first program listed in the editor elevate.toml(5) option is used.
    ...
    * Users are never allowed to edit device special files.
    ...
    If the specified file does not exist, it will be created. Unlike most commands run by elevate, the editor is run with the invoking user's environment unmodified. If the temporary file becomes empty after editing, the user will be prompted before it is installed.

`-u` *user*, `--user`=*user*
:   Run the *command* as another user than the default (**root**).

`-V`, `--version`
:   Display the current version of elevate.
...
`--`
:   Indicates the end of the elevate options and start of the *command*.
...
the user may set variables that would otherwise be forbidden. See [elevate.toml(5)](elevate.toml.5.md) for more information.

# SEE ALSO

[elev(1)](elev.1.md), [elevate.toml(5)](elevate.toml.5.md), [viselev(8)](viselev.8.md)
