...
use super::{ElevatePath, ElevateString};

#[derive(Debug)]
pub enum Error {
    Silent,
    NotAllowed {
        username: ElevateString,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::SelfCheckNoNewPrivs => {
                xlat_write!(
                    f,
                    "The \"no new privileges\" flag is set, which prevents elevate from running as root.\n\
                    If elevate is running in a container, you may need to adjust the container \
