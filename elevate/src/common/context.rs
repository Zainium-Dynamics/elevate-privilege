...
pub struct Context {
    ...
    // sudoedit
    pub files_to_edit: Vec<Option<ElevatePath>>,
}
...
impl Context {
    pub fn from_run_opts(
        elevate_options: ElevateRunOptions,
        policy: &mut Elevators,
        ...
        let command = {
            ...
            elevate_call(&target_user, &target_group, || {
                CommandAndArguments::build_from_args(shell, elevate_options.positional_args, path)
            })?
        };
        ...
        Ok(Context {
            ...
            target_user,
            target_group,
            use_session_records: !elevate_options.reset_timestamp,
            launch,
            chdir: elevate_options.chdir,
            askpass: elevate_options.askpass,
            stdin: elevate_options.stdin,
            bell: elevate_options.bell,
            background: elevate_options.background,
            prompt,
            non_interactive: elevate_options.non_interactive,
            files_to_edit: vec![],
        })
    }

    pub fn from_edit_opts(elevate_options: ElevateEditOptions) -> Result<Context, Error> {
        use std::path::Path;
        let hostname = Hostname::resolve();
        let current_user = CurrentUser::resolve()?;
        ...
        let resolved_args = elevate_call(&target_user, &target_group, || {
            elevate_options.positional_args.iter().map(|arg| {
        ...
        let files_to_edit = resolved_args
            .clone()
            .map(|path| path.ok().map(ElevatePath::from_cli_string))
            .collect();

        // if a path resolved to something that isn't in UTF-8, it means it isn't in the elevators file
        ...
        // TODO: the more Rust way of doing things would be to create an alternative for elevatedit instead;
        // but a stringly typed interface feels the most decent thing to do (if we can pull it off)
        // since "elevatedit" really is like a builtin command to elevate. We may want to be a bit 'better' than
        // ogsudo in the future.
        let command = CommandAndArguments {
            command: std::path::PathBuf::from("elevatedit"),
            arguments,
            ..Default::default()
        };
    ...
    pub fn from_list_opts(
        elevate_options: ElevateListOptions,
        policy: &mut Elevators,
    ) -> Result<Context, Error> {
        let hostname = Hostname::resolve();
        let current_user = CurrentUser::resolve()?;
        ...
        let command = if elevate_options.positional_args.is_empty() {
            Default::default()
        } else {
            let system_path;
...
#[cfg(test)]
mod tests {
    use crate::{common::resolve::CurrentUser, elevate::ElevateAction, system::Hostname};

    use super::Context;

    #[test]
    fn test_build_run_context() {
        let mut options = ElevateAction::try_parse_from(["sudo", "echo", "hello"])
            .unwrap()
            .try_into_run()
            .ok()
            .unwrap();
