...
pub(in crate::exec) fn exec_pty(
    ...
    if !io::stdin().is_terminal_for_pgrp(parent_pgrp) {
        ...
        if foreground && parent_pgrp != elevate_pid {
            // If elevate is not the process group leader and stdin is not a terminal we might be
...
impl ParentClosure {
    ...
    /// - is in the process group of the command and either elevate or the command is the leader.
    ...
    /// Suspend elevate if the command is suspended.
    ///
    ...
    fn suspend_pty(
        ...
        if let SIGTTOU | SIGTTIN = signal {
            // If elevate is already the foreground process we can resume the command in the
        ...
        if self.parent_pgrp != self.elevate_pid && kill(self.parent_pgrp, 0).is_err()
    ...
    /// Restore the terminal when elevate resumes after receiving `SIGCONT`.
