...
// NOTE: elevate-pam is NOT linked at build time (no #[link(name = "pam")]).
// At runtime `dynload` dlopen()s libelevate_pam.so* first (Zainium / elevate
// product). Classic libpam.so is only used if ELEVATE_ALLOW_LIBPAM=1.
// That keeps musl-static elevate free of link-time -lpam.
//
// See: src/pam/dynload.rs and docs/elevate-pam-integration.md
...
impl PamContext {
    ...
    pub fn authenticate(&mut self, for_user: &str) -> PamResult<()> {
        ...
        let auth_res = pam_err(unsafe { (dynload::pam_lib().pam_authenticate)(self.pamh, flags) });
    ...
    pub fn validate_account(&mut self) -> PamResult<()> {
        ...
        // SAFETY: `self.pamh` contains a correct handle (obtained from `pam_start`)
        pam_err(unsafe { (dynload::pam_lib().pam_acct_mgmt)(self.pamh, flags) })
    }
    ...
    pub fn get_user(&mut self) -> PamResult<String> {
        ...
        // SAFETY: `self.pamh` contains a correct handle (obtained from `pam_start`)
        pam_err(unsafe { (dynload::pam_lib().pam_get_item)(self.pamh, PAM_USER as _, &mut data) })?;
    ...
    fn credentials(&mut self, action: c_int) -> PamResult<()> {
        ...
        // SAFETY: `self.pamh` contains a correct handle (obtained from `pam_start`).
        pam_err(unsafe { (dynload::pam_lib().pam_setcred)(self.pamh, flags) })
    }
    ...
    pub fn change_auth_token(&mut self, expired_only: bool) -> PamResult<()> {
        ...
        // SAFETY: `self.pamh` contains a correct handle (obtained from `pam_start`).
        pam_err(unsafe { (dynload::pam_lib().pam_chauthtok)(self.pamh, flags) })
    }
    ...
    pub fn open_session(&mut self) -> PamResult<()> {
        ...
        // SAFETY: `self.pamh` contains a correct handle (obtained from `pam_start`).
        pam_err(unsafe { (dynload::pam_lib().pam_open_session)(self.pamh, self.silent_flag()) })?;
    ...
    pub fn env(&mut self) -> PamResult<Vec<(OsString, OsString)>> {
        ...
        let envs = unsafe { (dynload::pam_lib().pam_getenvlist)(self.pamh) };
        if envs.is_null() {
