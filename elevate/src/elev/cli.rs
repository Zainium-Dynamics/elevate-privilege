...
pub struct SuRunOptions {
    ...
    // -G
    pub supp_group: Vec<ElevateString>,
    // -w
    pub whitelist_environment: Vec<String>,

    pub user: ElevateString,
...
impl TryFrom<SuOptions> for SuRunOptions {
    ...
    fn try_from(mut opts: SuOptions) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            user: ElevateString::try_from(user).map_err(|err| err.to_string())?,
            arguments,
        })
    }
}
    ...
    const SU_OPTIONS: &'static [SuOption] = &[
        SuOption {
            ...
            set: |elevate_options, argument| {
                if argument.is_some() {
                    elevate_options.command = argument.map(OsString::from);
                    Ok(())
                } else {
        ...
        SuOption {
            ...
            set: |elevate_options, _| {
                if elevate_options.login {
                    Err(more_than_once("--login"))
                } else {
                    elevate_options.login = true;
                    Ok(())
                }
            },
        },
        SuOption {
            ...
            takes_argument: false,
            set: |elevate_options, _| {
                if elevate_options.preserve_environment {
                    Err(more_than_once("--preserve-environment"))
                } else {
                    elevate_options.preserve_environment = true;
                    Ok(())
                }
            },
        },
        ...
        SuOption {
            ...
            takes_argument: false,
            set: |elevate_options, _| {
                if elevate_options.pty {
                    Err(more_than_once("--pty"))
                } else {
                    elevate_options.pty = true;
                    Ok(())
                }
            },
        },
        SuOption {
            ...
            set: |elevate_options, argument| {
                if let Some(path) = argument {
                    elevate_options.shell = Some(PathBuf::from(path));
                    Ok(())
                } else {
        ...
        SuOption {
            ...
            set: |elevate_options, argument| {
                if let Some(list) = argument {
                    ...
                    elevate_options.whitelist_environment.extend(values);
                    Ok(())
                } else {
        ...
        SuOption {
            ...
            takes_argument: false,
            set: |elevate_options, _| {
                if elevate_options.version {
                    Err(more_than_once("--version"))
                } else {
                    elevate_options.version = true;
                    Ok(())
                }
            },
        },
        SuOption {
            ...
            takes_argument: false,
            set: |elevate_options, _| {
                if elevate_options.help {
                    Err(more_than_once("--help"))
                } else {
                    elevate_options.help = true;
                    Ok(())
                }
            },
        },
