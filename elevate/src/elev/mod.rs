...
fn run(options: SuRunOptions) -> Result<(), Error> {
    ...
    let run_options = context.as_run_options();
    ...
    let command_exit_reason = crate::exec::run_command(run_options, environment);
...
pub fn main() {
    crate::log::ElevateLogger::new("elev: ").into_global_logger();

    let action = match SuAction::from_env() {
        Ok(action) => action,
        Err(error) => {
            println_ignore_io_error!("elev: {error}\n{USAGE_MSG}");
            std::process::exit(1);
        }
    };
    ...
    match action {
        ...
        SuAction::Version(_) => {
            eprintln_ignore_io_error!("elev (zainium-elevators) {VERSION}");
            std::process::exit(0);
        }
        SuAction::Run(options) => match run(options) {
            Err(Error::CommandNotFound(c)) => {
                eprintln_ignore_io_error!("elev: {}", Error::CommandNotFound(c));
                std::process::exit(127);
            }
            Err(Error::InvalidCommand(c)) => {
                eprintln_ignore_io_error!("elev: {}", Error::InvalidCommand(c));
                std::process::exit(126);
            }
            Err(error) => {
                eprintln_ignore_io_error!("elev: {error}");
                std::process::exit(1);
            }
            _ => {}
        },
