...
use crate::elevators::Restrictions;
use crate::system::{PATH_MAX, audit::zoneinfo_path};

use super::wildcard_match::wildcard_match;

// TODO: use _PATH_STDPATH from paths.h
// NOTE: this build targets systems without a merged /usr (i.e. /bin, /sbin
// are NOT symlinks into /usr) -- so we intentionally omit /usr/bin and
// /usr/sbin here.
pub(crate) fn path_default() -> String {
    let paths = elevate_paths::get();
    format!("{}:/bin:{}:/sbin", paths.bindir, paths.sbindir)
}
...
/// Formats the command and arguments passed for the ELEVATE_COMMAND
...
/// Construct elevate-specific environment variables
fn add_extra_env(
    context: &Context,
    cfg: &Restrictions,
    elevate_ps1: Option<OsString>,
    environment: &mut Environment,
) {
    // current user
    environment.insert("ELEVATE_COMMAND".into(), format_command(&context.command));
    environment.insert(
        "ELEVATE_UID".into(),
    ...
    // set in elevators, or when the -s option is specified and set_home is set in elevators.
    ...
    match (
        ...
        // when the set_logname option is enabled in elevators, or when the env_reset option
        // is enabled in elevators (unless LOGNAME is present in the env_keep list).
    ...
    // The ELEVATE_PS1 variable requires special treatment as the PS1 variable must be set in the
    // target environment to the same value of ELEVATE_PS1 if the latter is set.
    if let Some(elevate_ps1_value) = elevate_ps1 {
        // set PS1 to the ELEVATE_PS1 value in the target environment
        environment.insert("PS1".into(), elevate_ps1_value);
    }
}
...
/// and the ELEVATE_* variables are set based on the invoking user.
///
...
pub fn get_target_environment(
    ...
    // retrieve ELEVATE_PS1 value to set a PS1 value as additional environment
    let elevate_ps1 = current_env.get(OsStr::new("ELEVATE_PS1")).cloned();
    ...
    add_extra_env(context, settings, elevate_ps1, &mut environment);
        ...
        pub fn check_should_keep(&self, key: &str, value: &str, expected: bool) {
            assert_eq!(
                should_keep(
                    OsStr::new(key),
                    OsStr::new(value),
                    &crate::elevators::Restrictions {
                        env_keep: &self.keep,
                        env_check: &self.check,
                        path: self.path.as_deref(),
                        chdir: crate::elevators::DirChange::Strict(None),
