...
> elevate env
    ...
    PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
    SHELL=/bin/bash
    ELEVATE_COMMAND=/usr/bin/env
    ELEVATE_GID=1000
    ELEVATE_UID=1000
    ELEVATE_USER=test
    ELEVATE_HOME=/home/test
    HOME=/root
    LOGNAME=root
    USER=root
    TERM=xterm
...
fn create_test_context(elevate_options: ElevateRunOptions) -> Context {
    ...
    Context {
        ...
        target_user: if elevate_options.user.as_deref() == Some("test") {
            current_user.into()
        } else {
            root_user
        },
        target_group: if elevate_options.user.as_deref() == Some("test") {
            current_group
        } else {
            root_group
        },
...
fn test_environment_variable_filtering() {
    ...
    for (cmd, expected_env) in parts {
        let options = ElevateAction::try_parse_from(cmd.split_whitespace())
            .unwrap()
            .try_into_run()
            .ok()
            .unwrap();
