...
use super::cli::{ElevateRunOptions, ElevateValidateOptions};
...
fn read_elevators() -> Result<Elevators, Error> {
    let elevators_path = &super::candidate_elevators_file();

    let (elevators, syntax_errors) = Elevators::open(elevators_path).map_err(|e| {
        // Provide a more helpful error message when the elevators file is missing
        if e.kind() == std::io::ErrorKind::NotFound {
            Error::Configuration(xlat!(
                "elevators file not found: {path}\n\
                 \n\
                 The elevator file is required for elevate to function. Please ensure:\n\
                 - The file exists at the expected location\n\
                 - You have the necessary permissions to read it\n\
                 - If setting up elevate for the first time, create a elevators file with appropriate permissions\n\
                 \n",
                path = elevators_path.display()
            ))
        } else {
...
fn judge(mut policy: Elevators, context: &Context) -> Result<Judgement, Error> {
    Ok(policy.check(
        &*context.current_user,
        &context.hostname,
        crate::elevators::Request {
...
pub fn run(mut cmd_opts: ElevateRunOptions) -> Result<(), Error> {
    let mut policy = read_elevators()?;
...
pub fn run_validate(cmd_opts: ElevateValidateOptions) -> Result<(), Error> {
    let mut policy = read_elevators()?;
