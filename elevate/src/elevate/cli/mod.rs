...
use crate::common::{DisplayOsStr, ElevatePath, ElevateString};
...
pub enum ElevateAction {
    Edit(ElevateEditOptions),
    Help(ElevateHelpOptions),
    List(ElevateListOptions),
    RemoveTimestamp(ElevateRemoveTimestampOptions),
    ResetTimestamp(ElevateResetTimestampOptions),
    Run(ElevateRunOptions),
    Validate(ElevateValidateOptions),
    Version(ElevateVersionOptions),
}

pub(super) fn is_elevatedit(command_path: Option<OsString>) -> bool {
    std::path::Path::new(&command_path.unwrap_or_default())
        .file_name()
        .is_some_and(|name| name.as_encoded_bytes().starts_with(b"elevatedit"))
}
...
impl ElevateAction {
    ...
    {
        let opts = ElevateOptions::try_parse_from(iter)?;
        opts.validate()
    }
}

// sudo -h | -K | -k | -V
pub struct ElevateHelpOptions {}

impl TryFrom<ElevateOptions> for ElevateHelpOptions {
    type Error = String;

    fn try_from(mut opts: ElevateOptions) -> Result<Self, Self::Error> {
        // see `ElevateOptions::validate`
...
// sudo -h | -K | -k | -V
pub struct ElevateVersionOptions {}

impl TryFrom<ElevateOptions> for ElevateVersionOptions {
    type Error = String;
...
// sudo -h | -K | -k | -V
pub struct ElevateRemoveTimestampOptions {}

impl TryFrom<ElevateOptions> for ElevateRemoveTimestampOptions {
    type Error = String;
...
// sudo -h | -K | -k | -V
pub struct ElevateResetTimestampOptions {}

impl TryFrom<ElevateOptions> for ElevateResetTimestampOptions {
    type Error = String;
...
pub struct ElevateValidateOptions {
    ...
    // -g
    pub group: Option<ElevateString>,
    // -u
    pub user: Option<ElevateString>,
}

impl TryFrom<ElevateOptions> for ElevateValidateOptions {
    type Error = String;
...
impl TryFrom<ElevateOptions> for ElevateEditOptions {
    type Error = String;
...
pub struct ElevateListOptions {
    ...
    pub group: Option<ElevateString>,
    // -U
    pub other_user: Option<ElevateString>,
    // -u
    pub user: Option<ElevateString>,
...
impl TryFrom<ElevateOptions> for ElevateListOptions {
    type Error = String;
...
impl TryFrom<ElevateOptions> for ElevateRunOptions {
    type Error = String;
...
struct ElevateOptions {
    ...
    background: bool,
    // -D
    chdir: Option<ElevatePath>,
    // -g
    group: Option<ElevateString>,
    // -i
    login: bool,
    // -n
    non_interactive: bool,
    // -U
    other_user: Option<ElevateString>,
    // -E
    /* ignored, part of env_var_list */
    // -s
    shell: bool,
...
impl ElevateArg {
        ...
        I: Iterator<Item = OsString>,
    {
        ...
        while let Some(arg) = arg_iter.next() {
            if arg == "--" {
                processed.push(ElevateArg::Rest(arg_iter.collect()));
                break;
                ...
                if let Some((key, value)) = unprefixed.split_once('=') {
                    ...
                    if let Some(next) = arg_iter.next() {
                        ...
                        processed.push(ElevateArg::Argument(arg, next.to_owned()));
                    } else {
                        Err(xlat!("'{option}' expects an argument", option = arg))?;
                    }
                } else {
                    let arg = String::from_utf8(arg.into_encoded_bytes()).expect("already checked");
                    processed.push(ElevateArg::Flag(arg));
                }
            } else if let Some(unprefixed) = arg.as_encoded_bytes().strip_prefix(b"-") {
                let unprefixed = str::from_utf8(unprefixed).map_err(|_| demand_utf8(&arg))?;
                ...
                while let Some(curr) = chars.next() {
                    ...
                    if Self::TAKES_ARGUMENT_SHORT.contains(&curr) {
                        ...
                        if next.is_some() {
                            processed.push(ElevateArg::Argument(flag, rest.to_string()));
                ...
                processed.push(ElevateArg::Environment(key, value));
            } else {
                let mut rest = vec![arg];
                rest.extend(arg_iter);
                processed.push(ElevateArg::Rest(rest));
                break;
            }
        }
...
impl ElevateOptions {
    fn validate(self) -> Result<ElevateAction, String> {
        let action = if self.help {
            ElevateAction::Help(self.try_into()?)
        } else if self.version {
            ElevateAction::Version(self.try_into()?)
        } else if self.remove_timestamp {
            ElevateAction::RemoveTimestamp(self.try_into()?)
        } else if self.validate {
            ElevateAction::Validate(self.try_into()?)
        } else if self.list.is_some() {
            ElevateAction::List(self.try_into()?)
        } else if self.edit {
            ElevateAction::Edit(self.try_into()?)
        } else {
            let is_run = self.login | self.shell | !self.positional_args.is_empty();

            if is_run {
                ElevateAction::Run(self.try_into()?)
            } else if self.reset_timestamp {
                ElevateAction::ResetTimestamp(self.try_into()?)
            } else {
        ...
        T: Into<OsString> + Clone,
    {
        ...
        let invoked_as_elevatedit = is_elevatedit(arg_iter.next());

        let mut options = Self {
            edit: invoked_as_elevatedit,
            ..Self::default()
        };

        let arg_iter = ElevateArg::normalize_arguments(arg_iter)?
            .into_iter()
            .peekable();

        for arg in arg_iter {
            match arg {
                ElevateArg::Flag(flag) => match flag.as_str() {
                    "-A" | "--askpass" => {
                    ...
                    "-e" | "--edit" if !invoked_as_elevatedit => {
                        options.edit = true;
                    }
                    "-H" | "--set-home" => {
                ...
                ElevateArg::Argument(option, value) => match option.as_str() {
                    "-D" | "--chdir" => {
                        options.chdir = Some(ElevatePath::from_cli_string(value));
                    }
                    "-E" | "--preserve-env" => {
                        options
                            .env_var_list
                    ...
                    "-g" | "--group" => {
                        options.group = Some(ElevateString::from_cli_string(value));
                    }
                    "-p" | "--prompt" => {
                        options.prompt = Some(value);
                    }
                    "-U" | "--other-user" => {
                        options.other_user = Some(ElevateString::from_cli_string(value));
                    }
                    "-u" | "--user" => {
                        options.user = Some(ElevateString::from_cli_string(value));
                    }
                    _option => {
                        Err(xlat!("invalid option provided"))?;
                    }
                },
                ElevateArg::Environment(key, value) => {
                    options.env_var_list.push((key, value));
                }
                ElevateArg::Rest(mut rest) => {
...
fn reject_all(context: &str, opts: ElevateOptions) -> Result<(), String> {
    macro_rules! check_options {
        ($($field:ident $(= $name:expr)?,)*) => {{
            let ElevateOptions { $($field),* } = opts;
