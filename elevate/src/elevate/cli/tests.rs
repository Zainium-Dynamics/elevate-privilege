use crate::common::ElevatePath;

use super::{ElevateAction, ElevateOptions, ElevateRunOptions};
...
impl ElevateAction {
    ...
    pub fn try_into_run(self) -> Result<ElevateRunOptions, Self> {
...
fn short_preserve_env_with_var_fails() {
    ...
    for args in argss {
        let res = ElevateOptions::try_parse_from(args);
        assert!(res.is_err())
    }
}
...
fn preserve_env_with_var() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "--preserve-env=HOME"]).unwrap();
    assert_eq!(
...
fn preserve_env_with_several_vars() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "--preserve-env=PATH,HOME"]).unwrap();
    assert_eq!(
        [
...
fn preserve_env_boolean_and_list() {
    ...
    for args in argss {
        let cmd = ElevateOptions::try_parse_from(args).unwrap();
        assert_eq!(
            [("HOME".to_string(), std::env::var("HOME").unwrap())],
            cmd.env_var_list.as_slice()
        );
...
#[test]
fn env_variable() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "ENV=with_a_value"]).unwrap();
    assert_eq!(
        cmd.env_var_list,
...
fn several_env_variables() {
    let cmd = ElevateOptions::try_parse_from([
        "sudo",
        "ENV=with_a_value",
...
fn mix_env_variables_with_trailing_args_divided_by_known_flag() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-i", "external=args", "something"]).unwrap();
    assert_eq!(
        cmd.env_var_list,
...
fn first_trailing_env_var_is_not_an_external_arg() {
    let cmd = ElevateAction::try_parse_from(["sudo", "FOO=1", "command", "BAR=2"]).unwrap();
    let opts = if let ElevateAction::Run(opts) = cmd {
        opts
    } else {
        panic!()
    };
...
fn single_env_var_declaration() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "FOO=1", "command"]).unwrap();
...
fn shorthand_with_argument() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-u", "ferris"]).unwrap();
    assert_eq!(cmd.user.as_deref(), Some("ferris"));
}

#[test]
fn shorthand_with_direct_argument() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-uferris"]).unwrap();
    assert_eq!(cmd.user.as_deref(), Some("ferris"));
}

#[test]
fn shorthand_without_argument() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-u"]);
    assert!(cmd.is_err())
}

#[test]
fn non_interactive() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-n"]).unwrap();
    assert!(cmd.non_interactive);

    let cmd = ElevateOptions::try_parse_from(["sudo", "--non-interactive"]).unwrap();
    assert!(cmd.non_interactive);
}

#[test]
fn stdin() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-S"]).unwrap();
    assert!(cmd.stdin);

    let cmd = ElevateOptions::try_parse_from(["sudo", "--stdin"]).unwrap();
    assert!(cmd.stdin);
}
...
fn shell() {
    ...
    let cmd = ElevateOptions::try_parse_from(["sudo", "--shell"]).unwrap();
    assert!(cmd.shell);
}

#[test]
fn directory() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-D/some/path"]).unwrap();
    assert_eq!(cmd.chdir, Some(ElevatePath::from("/some/path")));

    let cmd = ElevateOptions::try_parse_from(["sudo", "--chdir", "/some/path"]).unwrap();
    assert_eq!(cmd.chdir, Some(ElevatePath::from("/some/path")));
...
#[test]
fn group() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-grustaceans"]).unwrap();
    assert_eq!(cmd.group.as_deref(), Some("rustaceans"));

    let cmd = ElevateOptions::try_parse_from(["sudo", "--group", "rustaceans"]).unwrap();
    assert_eq!(cmd.group.as_deref(), Some("rustaceans"));
...
fn other_user() {
    ...
    let cmd = ElevateOptions::try_parse_from(["sudo", "--other-user", "ferris"]).unwrap();
    assert_eq!(cmd.other_user.as_deref(), Some("ferris"));
...
#[test]
fn invalid_option() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "--wololo"]);
    assert!(cmd.is_err())
}

#[test]
fn invalid_option_with_argument() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "--background=yes"]);
    assert!(cmd.is_err())
}

#[test]
fn no_argument_provided() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "--user"]);
    assert!(cmd.is_err())
}

#[test]
fn login() {
    let cmd = ElevateOptions::try_parse_from(["sudo", "-i"]).unwrap();
    assert!(cmd.login);

    let cmd = ElevateOptions::try_parse_from(["sudo", "--login"]).unwrap();
    assert!(cmd.login);
}

#[test]
fn edit() {
    let cmd = ElevateAction::try_parse_from(["sudo", "-e", "filepath"]).unwrap();
    assert!(cmd.is_edit());

    let cmd = ElevateAction::try_parse_from(["sudo", "--edit", "filepath"]).unwrap();
    assert!(cmd.is_edit());

    let cmd = ElevateAction::try_parse_from(["elevatedit", "filepath"]).unwrap();
    assert!(cmd.is_edit());

    let res = ElevateAction::try_parse_from(["sudo", "--edit"]);
    assert!(res.is_err());

    let res = ElevateAction::try_parse_from(["elevatedit", "--edit", "filepath"]);
    assert!(res.is_err());
}

#[test]
fn help() {
    let cmd = ElevateAction::try_parse_from(["sudo", "-h"]).unwrap();
    assert!(cmd.is_help());

    let cmd = ElevateAction::try_parse_from(["sudo", "-zh"]);
    assert!(cmd.is_err());

    let cmd = ElevateAction::try_parse_from(["sudo", "--help"]).unwrap();
    assert!(cmd.is_help());
}

#[test]
fn conflicting_arguments() {
    let cmd = ElevateAction::try_parse_from(["sudo", "-K", "-k"]);
    assert!(cmd.is_err());

    let cmd = ElevateAction::try_parse_from(["sudo", "--remove-timestamp", "--reset-timestamp"]);
    assert!(cmd.is_err());

    let cmd = ElevateAction::try_parse_from(["sudo", "-K"]).unwrap();
...
fn list() {
    ...
    for args in valid {
        let cmd = ElevateAction::try_parse_from(args.iter().copied()).unwrap();
        assert!(cmd.is_list());
    }
    ...
    for args in invalid {
        let res = ElevateAction::try_parse_from(args.iter().copied());
        assert!(res.is_err())
    }
}
...
#[test]
fn validate() {
    let cmd = ElevateAction::try_parse_from(["sudo", "-v"]).unwrap();
    assert!(cmd.is_validate());

    let cmd = ElevateAction::try_parse_from(["sudo", "--validate"]).unwrap();
    assert!(cmd.is_validate());
}
...
fn version() {
    ...
    let cmd = ElevateAction::try_parse_from(["sudo", "--version"]).unwrap();
    assert!(cmd.is_version());
}

#[test]
fn run_reset_timestamp_command() {
    let action = ElevateAction::try_parse_from(["sudo", "-k", "true"])
        .unwrap()
        .try_into_run()
        .ok()
        .unwrap();
...
fn run_reset_timestamp_login() {
    let action = ElevateAction::try_parse_from(["sudo", "-k", "-i"])
        .unwrap()
        .try_into_run()
        .ok()
        .unwrap();
...
fn run_reset_timestamp_shell() {
    let action = ElevateAction::try_parse_from(["sudo", "-k", "-s"])
        .unwrap()
        .try_into_run()
        .ok()
        .unwrap();
...
#[test]
fn run_no_command() {
    assert!(ElevateAction::try_parse_from(["sudo", "-u", "root"]).is_err());
}
