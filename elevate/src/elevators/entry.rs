...
use crate::{
    common::{DisplayOsStr, ElevateString, resolve::CurrentUser},
...
fn root_runas() -> RunAs {
    let name = User::from_uid(UserId::ROOT)
        ...
        .flatten()
        .map(|u| u.name)
        .unwrap_or(ElevateString::new("root".into()).unwrap());
...
fn write_tag(
    ...
    if tag.apparmor_profile != last_tag.apparmor_profile {
        ...
        let profile = tag
            .apparmor_profile
            .as_ref()
            .expect("elevators spec turned off");
        f.write_str(profile)?;
        f.write_str(" ")?;
    }
    ...
    if tag.cwd != last_tag.cwd {
        f.write_str("CWD=")?;
        match tag.cwd.as_ref().expect("elevators spec turned off") {
    ...
    match meta {
        ...
        Meta::Only((cmd, args)) => {
            ...
            match args {
                Args::Exact(args) => {
                    for arg in args {
                        write!(f, " {}", DisplayOsStr(arg.as_os_str()))?;
                    }
                    if args.is_empty() {
                        write!(f, " \"\"")?;
                    }
