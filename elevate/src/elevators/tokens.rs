...
use crate::common::{ElevatePath, ElevateString};
...
pub struct Username(pub ElevateString);
...
/// (Maybe this is better defined not as a Token but simply directly as an implementation of [crate::elevators::basic_parser::Parse])
...
#[repr(u32)]
pub enum ChDir {
    Path(ElevatePath) = HARDENED_ENUM_VALUE_0,
...
impl Token for ChDir {
    ...
    fn construct(s: String) -> Result<Self, String> {
        if s == "*" {
            ...
            Ok(ChDir::Path(
                ElevatePath::try_from(s).map_err(|e| e.to_string())?,
            ))
        }
    }
