...
use super::tokens::*;
use crate::common::ElevateString;
use crate::common::{
...
/// The Elevators file allows negating items with the exclamation mark.
...
#[repr(u32)]
pub enum Identifier {
    Name(ElevateString) = HARDENED_ENUM_VALUE_0,
...
/// Commands in /etc/elevators can have attributes attached to them, such as NOPASSWD, NOEXEC, ...
...
/// The main AST object for one elevator_rule-permission line
...
/// The Elevators file can contain permissions and directives
#[repr(u32)]
pub enum ElevatorLine {
...
fn parse_meta<T: Parse>(
    ...
    embed: impl FnOnce(ElevateString) -> T,
) -> Parsed<Meta<T>> {
...
/// where the elevator_rule grammar isn't nicely LL(1); so at the same place where "NOPASSWD" can appear,
...
/// elevator_rule line (separated by ":")
...
/// elevator_rule line (separated by ","); there is some ambiguity in the original grammar:
    ...
    fn parse(stream: &mut CharStream) -> Parsed<ElevatorLine> {
        ...
        if stream.peek() == Some('#') {
            return if let Ok(ident) = try_nonterminal::<Identifier>(stream) {
                ...
                make(ElevatorLine::Spec(PermissionSpec { users, permissions }))
            } else {
                // the failed "try_nonterminal::<Identifier>" will have consumed the '#'
                // the most ignominious part of elevators: having to parse bits of comments
                parse_include(stream).or_else(|_| {
                    stream.skip_to_newline();
                    make(ElevatorLine::LineComment)
                })
            };
        }
        ...
        if stream.peek() == Some('"') {
            ...
            if let Some(directive) = maybe(get_directive(key, stream, start_state))? {
                ...
                make(ElevatorLine::Decl(directive))
            } else {
                let permissions = expect_nonterminal(stream)?;
                make(ElevatorLine::Spec(PermissionSpec { users, permissions }))
            }
...
fn parse_include(stream: &mut CharStream) -> Parsed<ElevatorLine> {
    ...
    let result = match try_nonterminal(stream)? {
        Some(Username(key)) if key == "include" => {
            ...
            ElevatorLine::Include(path, span)
        }
