...
//! Code that checks (and in the future: lists) permissions in the elevators file
...
/// cannot be determined simply by reading a elevators configuration. This is used
...
/// This function takes a file argument for a elevators file and processes it.
impl Elevators {
    pub fn open(path: impl AsRef<Path>) -> Result<(Elevators, Vec<Error>), io::Error> {
        let elevators = open_elevators(path.as_ref())?;
        Ok(analyze(path.as_ref(), elevators))
    }

    pub fn read<R: io::Read, P: AsRef<Path>>(
        reader: R,
        path: P,
    ) -> Result<(Elevators, Vec<Error>), io::Error> {
        let elevators = read_elevators(reader)?;
        Ok(analyze(path.as_ref(), elevators))
    }
    ...
    pub(crate) fn viselev_editor_path<User: UnixUser + PartialEq<User>>(
        mut self,
        on_host: &system::Hostname,
        am_user: &User,
        target_user: &User,
...
fn select_editor(settings: &Settings, trusted_env: bool) -> Option<(PathBuf, Vec<OsString>)> {
    ...
    for key in ["ELEVATE_EDITOR", "VISUAL", "EDITOR"] {
        if let Ok(editor_env) = std::env::var(key) {
            ...
            let editor = if is_valid_executable(&editor) {
                ...
                &editor,
                &std::env::var("PATH").unwrap_or(crate::elevate::path_default()),
            ) {
                editor
            } else {
                continue;
...
fn group_cmd_specs_per_runas<'a>(
    ...
    //
    // But sudo --list also outputs lines that are from different lines in the elevators file on
...
fn open_elevators(path: &Path) -> io::Result<Vec<basic_parser::Parsed<ElevatorLine>>> {
    let source = audit::secure_open_elevators(path)?;
    read_elevators(source)
}

#[cfg(feature = "unstable-remote-sudoers")]
fn open_remote_elevators(path: &Path) -> io::Result<Vec<basic_parser::Parsed<ElevatorLine>>> {
    let source = audit::secure_open_remote_elevators(path)?;
    read_elevators(source)
}
...
/// user/group. Not that in the elevators file, later permissions override earlier restrictions.
...
fn check_permission<User: UnixUser + PartialEq<User>, Group: UnixGroup>(
    ...
    let matching_user_specs = elevators.matching_user_specs(am_user, on_host).flatten();
...
/// negative (i.e. preceeded by an even number of exclamation marks in the elevators file)
...
/// Process a elevators-parsing file into a workable AST
fn analyze(
    path: &Path,
    elevators: impl IntoIterator<Item = basic_parser::Parsed<ElevatorLine>>,
) -> (Elevators, Vec<Error>) {
    use crate::common::{HARDENED_ENUM_VALUE_0, HARDENED_ENUM_VALUE_1, HARDENED_ENUM_VALUE_2};
    use Directive::*;

    let mut result: Elevators = Default::default();
    ...
    fn resolve_relative(base: &Path, path: impl AsRef<Path>) -> PathBuf {
        if path.as_ref().is_relative() {
            // there should always be a parent since we start with /etc/elevators, and make every other path
        ...
        if include_state.limit_reached() {
            ...
            let (res, next_state, kind) = match include_source {
                ...
                IncludeDirective::Remote => (
                    open_remote_elevators(path),
                    &mut IncludeState::Forbidden,
                    "socket",
                ),
                _ => (open_elevators(path), include_state.inc(), "file"),
            };

            match res {
                Ok(subelevator) => process(cfg, path, subelevator, diagnostics, next_state),
                Err(e) => {
                    let message = if e.kind() == io::ErrorKind::NotFound {
                        // improve the error message in this case
                        format!("cannot open elevators {} '{}'", kind, path.display())
                    } else {
                        e.to_string()
                    };
    ...
    fn process(
        ...
        for item in elevators {
            match item {
                Ok(line) => match line {
                    ElevatorLine::LineComment => {}

                    ElevatorLine::Spec(permission) => {
                    ...
                    ElevatorLine::Decl(HostAlias(mut def)) => cfg.aliases.host.1.append(&mut def),
                    ElevatorLine::Decl(UserAlias(mut def)) => cfg.aliases.user.1.append(&mut def),
                    ElevatorLine::Decl(RunasAlias(mut def)) => cfg.aliases.runas.1.append(&mut def),
                    ElevatorLine::Decl(CmndAlias(mut def)) => cfg.aliases.cmnd.1.append(&mut def),

                    ElevatorLine::Decl(Defaults(params, scope)) => {
                    ...
                    ElevatorLine::Include(path, span) => include(
                        cfg,
                        cur_path,
                        span,
                    ...
                    ElevatorLine::IncludeDir(path, span) => {
                        if path.contains("%h") {
                            diagnostics.push(Error {
                                source: Some(cur_path.to_owned()),
                                location: Some(span),
                                message: format!(
                                    "cannot open elevators file {path}: \
                        ...
                        let Ok(files) = std::fs::read_dir(&path) else {
                            diagnostics.push(Error {
                                source: Some(cur_path.to_owned()),
                                location: Some(span),
                                message: format!("cannot open elevators file {}", path.display()),
                            });
                            continue;
                        };
...
/// Alias definition inin a Elevators file can come in any order; and aliases can refer to other aliases, etc.
