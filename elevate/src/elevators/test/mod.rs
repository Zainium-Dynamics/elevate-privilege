...
// alternative to parse_eval, but goes through elevator_rule! directly
#[must_use]
fn parse_line(s: &str) -> ElevatorLine {
    elevator_rule![s].next().unwrap().unwrap()
}

/// Returns `None` if a syntax error is encountered
fn try_parse_line(s: &str) -> Option<ElevatorLine> {
...
#[test]
fn ambiguous_spec() {
    assert!(parse_eval::<ast::ElevatorLine>("marc, User_Alias ALL = ALL").is_spec());
}
...
fn permission_test() {
    ...
    macro_rules! FAIL {
        ([$($sudo:expr),*], $user:expr => $req:expr, $server:expr; $command:expr) => {
            let (Elevators { rules,aliases,settings, customisers }, _) = analyze(Path::new("/etc/fakesudoers"), elevator_rule![$($sudo),*]);
            ...
            assert_eq!(Elevators { rules, aliases, settings, customisers }.check(&Named($user), &system::Hostname::fake($server), req).flags, None);
        }
    }
    ...
    macro_rules! pass {
        ([$($sudo:expr),*], $user:expr => $req:expr, $server:expr; $command:expr $(=> [$($key:ident : $val:expr),*])?) => {
            ...
            let cmdvec = $command.split_whitespace().map(OsString::from).collect::<Vec<_>>();
            let req = Request { user: $req.0, group: $req.1, command: &realpath(cmdvec[0].as_ref()), arguments: &cmdvec[1..].to_vec() };
            let result = Elevators { rules, aliases, settings, customisers }.check(&Named($user), &system::Hostname::fake($server), req).flags;
    ...
    macro_rules! SYNTAX {
        ([$sudo:expr]) => {
            assert!(parse_string::<ElevatorLine>($sudo).is_err())
        };
    }
...
fn default_bool_test() {
    ...
    elevators.specify_host_user_runas(
        &system::Hostname::fake("host"),
        &Named("user"),
        Some(&Named("root")),
    );

    assert!(!elevators.settings.env_editor());
    assert!(elevators.settings.use_pty());
    assert!(elevators.settings.env_keep().is_empty());
    assert_eq!(elevators.settings.secure_path(), None);
    assert!(!elevators.settings.env_editor());
}
...
fn default_set_test() {
    ...
    assert_eq!(
        elevators.settings.env_keep(),
        &["FOO", "BAR"].into_iter().map(|x| x.to_string()).collect()
    );
    assert_eq!(
        elevators.settings.env_check(),
        &["FOO", "XYZZY"]
            .into_iter()
            .map(|x| x.to_string())
            .collect()
    );
    assert_eq!(elevators.settings.secure_path(), Some("/etc"));
    assert_eq!(elevators.settings.passwd_tries(), 5);

    assert!(parse_string::<ElevatorLine>("Defaults verifypw = \"sometimes\"").is_err());
    assert!(parse_string::<ElevatorLine>("Defaults verifypw = sometimes").is_err());
    assert!(parse_string::<ElevatorLine>("Defaults verifypw = never").is_ok());

    assert!(parse_string::<ElevatorLine>("Defaults runcwd = *").is_ok());
    assert!(parse_string::<ElevatorLine>("Defaults runcwd = /usr/local").is_ok());
    assert!(parse_string::<ElevatorLine>("Defaults !runcwd").is_ok());
}
...
fn invalid_directive() {
    parse_eval::<ast::ElevatorLine>("User_Alias, user Alias = user1, user2");
}

#[test]
#[should_panic = "embedded $ in username"]
fn invalid_username() {
    parse_eval::<ast::ElevatorLine>("User_Alias FOO = $dollar");
}

#[test]
#[should_panic = "wildcards are not allowed in command arguments"]
fn wildcard_in_argument() {
    parse_eval::<ast::ElevatorLine>("user ALL=/bin/hello w*");
}
...
fn directive_test() {
    ...
    match parse_eval::<ast::ElevatorLine>("User_Alias HENK = user1, user2") {
        ElevatorLine::Decl(Directive::UserAlias(defs)) => {
    ...
    match parse_eval::<ast::ElevatorLine>("Runas_Alias FOO = foo : BAR = bar") {
        ElevatorLine::Decl(Directive::RunasAlias(defs)) => {
...
fn gh676_percent_h_escape_unsupported() {
    let (_, errs) = analyze(
        Path::new("/etc/fakesudoers"),
        elevator_rule!(r#"@includedir "/etc/%h" "#),
    );
    assert_eq!(errs.len(), 1);
    assert_eq!(
        errs[0].message,
        "cannot open elevators file /etc/%h: percent escape %h in includedir is unsupported"
    );
    assert_eq!(
        errs[0].location,
        Some(Span {
...
fn default_specific_test() {
    ...
    let (mut base_elevators, _) = elevators();
    base_elevators.specify_host_user_runas(
        &system::Hostname::fake("generic"),
        &Named("generic"),
        Some(&Named("generic")),
    );

    assert!(base_elevators.settings.env_editor());
    assert!(!base_elevators.settings.use_pty());
    assert!(base_elevators.settings.env_keep().contains("COLORS"));
    assert_eq!(base_elevators.settings.secure_path(), None);

    let (mut mod_elevators, _) = elevators();
    mod_elevators.specify_host_user_runas(
        &system::Hostname::fake("host"),
        &Named("user"),
        Some(&Named("root")),
    );
    assert!(!mod_elevators.settings.env_editor());
    assert!(mod_elevators.settings.use_pty());
    assert!(mod_elevators.settings.env_keep().contains("COLORS"));
    assert_eq!(mod_elevators.settings.secure_path(), None);
    ...
    assert!(!mod_elevators.settings.use_pty());
    assert!(mod_elevators.settings.env_keep().contains("COLORS"));
    assert_eq!(mod_elevators.settings.secure_path(), Some("/bin"));
    mod_elevators.specify_command(Path::new("/bin/foo"), &["".into(), "a".into()]);
    assert!(mod_elevators.settings.env_keep().is_empty());
    ...
    mod_elevators.specify_command(Path::new("/usr/bin/rr"), &["thrice".into()]);
    assert!(mod_elevators.settings.env_editor());
    assert!(!mod_elevators.settings.use_pty());
    assert!(mod_elevators.settings.env_keep().contains("COLORS"));
    assert_eq!(mod_elevators.settings.secure_path(), None);
    ...
    let (mut mod_elevators, _) = elevators();
    mod_elevators.specify_command(Path::new("/usr/bin/rr"), &["twice".into()]);
    assert!(mod_elevators.settings.use_pty());
}
...
fn useralias_underscore_regression() {
    ...
    let spec = sudo.as_spec().expect("`ElevatorLine::Spec`");
    assert!(
        spec.users[0]
            .as_allow()
...
fn regression_check_recursion() {
    let (_, error) = analyze(
        Path::new("/etc/fakesudoers"),
        elevator_rule!["User_Alias A=user, B", "User_Alias B=A"],
    );
