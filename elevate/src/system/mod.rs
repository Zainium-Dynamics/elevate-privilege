...
compile_error!("elevate only works on Linux and FreeBSD");
...
pub struct User {
    pub uid: UserId,
    pub gid: GroupId,
    pub name: ElevateString,
    pub home: ElevatePath,
    pub shell: PathBuf,
    ...
    unsafe fn from_libc(pwd: &libc::passwd) -> Result<User, Error> {
        ...
        unsafe {
            Ok(User {
                ...
                name: ElevateString::new(string_from_ptr(pwd.pw_name))?,
                home: ElevatePath::new(os_string_from_ptr(pwd.pw_dir).into())?,
...
mod tests {
    ...
    pub(super) fn tempfile() -> std::io::Result<std::fs::File> {
        ...
        let filename = format!("elevate_test_{pid}_{timestamp}");
