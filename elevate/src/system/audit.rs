...
pub fn irrevocably_drop_privileges() {
    ...
    unsafe {
        ...
        // or in elevate_call (which always resets the user state after the closure finishes).
...
pub fn secure_open_elevators(path: impl AsRef<Path>) -> io::Result<File> {
...
pub fn secure_open_remote_elevators(path: impl AsRef<Path>) -> io::Result<BufReader<UnixStream>> {
...
pub fn zoneinfo_path() -> Option<String> {
    // NOTE: this build targets ZainiumOS — no /bin, /sbin, /lib, /etc at
    // root at all; everything lives under the configured prefix. We check
    // <prefix>/etc/zoneinfo first (alongside .../etc/localtime) and
    // <prefix>/lib/zoneinfo as a fallback.
    elevate_paths::get().zoneinfo_dirs().into_iter().find(|p| {
        let path = Path::new(p);
...
pub fn secure_open_for_elevatedit(
    ...
    fn secure_open_is_predictable() {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if std::fs::File::open("/var/log/wtmp").is_ok() {
                assert!(secure_open_elevators("/var/log/wtmp").is_err());
            }
        }

        // /etc/shadow should not be readable
        assert!(std::fs::File::open("/etc/shadow").is_err());
        assert!(secure_open_elevators("/etc/shadow").is_err());
    }
