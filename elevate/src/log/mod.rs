...
// logger_macro!(auth_error is Error to "elevate::auth");
logger_macro!(auth_warn is Warn to "elevate::auth" with format_args);
logger_macro!(auth_info is Info to "elevate::auth" with format_args);
// logger_macro!(auth_debug is Debug to "elevate::auth" with format_args);

logger_macro!(user_error is Error to "elevate::user" with xlat);
logger_macro!(user_warn is Warn to "elevate::user" with xlat);
logger_macro!(user_info is Info to "elevate::user" with xlat);
// logger_macro!(user_debug is Debug to "elevate::user" with xlat);
...
dev_logger_macro!(dev_error is Error to "elevate::dev");
dev_logger_macro!(dev_warn is Warn to "elevate::dev");
dev_logger_macro!(dev_info is Info to "elevate::dev");
dev_logger_macro!(dev_debug is Debug to "elevate::dev");
//dev_logger_macro!(dev_trace is Trace to "elevate::dev");

pub static LOGGER: OnceLock<ElevateLogger> = OnceLock::new();

#[derive(Default)]
pub struct ElevateLogger(Vec<(String, Box<dyn Log>)>);

impl ElevateLogger {
    pub fn new(prefix: &'static str) -> Self {
        let mut logger: Self = Default::default();

        logger.add_logger("elevate::auth", Syslog);

        logger.add_logger("elevate::user", SimpleLogger::to_stderr(prefix));

        #[cfg(feature = "dev")]
        {
            let path = option_env!("ELEVATE_DEV_LOGS")
                .map(|s| s.into())
                .unwrap_or_else(|| {
                    std::env::temp_dir().join(format!("elevate-dev-{}.log", std::process::id()))
                });
            logger.add_logger("elevate::dev", SimpleLogger::to_file(path, "").unwrap());
        }
...
#[cfg(test)]
mod tests {
    use super::ElevateLogger;

    #[test]
    fn can_construct_logger() {
        let logger = ElevateLogger::new("elevate: ");
