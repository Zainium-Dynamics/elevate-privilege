    ...
    for i in 0..argc as isize {
        unsafe {
            ...
            if let Ok(s) = std::ffi::CStr::from_ptr(p).to_str() {
                if let Some(v) = s.strip_prefix("item=") {
                    item = v.to_string();
                } else if let Some(v) = s.strip_prefix("sense=") {
                    sense = v.to_string();
                } else if let Some(v) = s.strip_prefix("file=") {
                    file = v.to_string();
                } else if let Some(v) = s.strip_prefix("onerr=") {
                    onerr = v.to_string();
                }
            }
        }
