//! Shared module `pam_pwhistory.so` for elevate-pam.
...
fn args_from_c(argc: c_int, argv: *const *const c_char) -> Vec<String> {
    let mut out = Vec::new();
    if argv.is_null() || argc <= 0 { return out; }
...
unsafe fn pamh_mut<'a>(pamh: *mut c_void) -> Option<&'a mut PamHandle> {
    if pamh.is_null() { None } else { Some(&mut *(pamh as *mut PamHandle)) }
}
...
macro_rules! sm {
    ($name:ident, $method:ident) => {
        ...
        pub unsafe extern "C" fn $name(
            pamh: *mut c_void, flags: c_int, argc: c_int, argv: *const *const c_char,
        ) -> c_int {
            ...
            let id = ModuleId::normalize("unix"); // fallback / integrate
            let Some(hooks) = elevate_pam::module::global::get(&id) else {
                return PAM_MODULE_UNKNOWN;
            };
            hooks.$method.map(|f| f(h, flags, &args).code()).unwrap_or(PAM_SUCCESS)
        }
    };
}
